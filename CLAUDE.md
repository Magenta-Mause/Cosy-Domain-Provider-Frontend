...
- **Styling: Tailwind utility classes first.** Prefer `className="..."` over `style={{...}}`. Only use inline styles for values that cannot be expressed as Tailwind classes (e.g. dynamic CSS variables like `color: "oklch(...)"`, or one-off numeric values with no Tailwind equivalent).
